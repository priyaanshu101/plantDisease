...
# 👇 Import your RAG directly
from query import Retriever, ask
...
@app.post("/suggestion")
async def suggest(request: DiseaseRequest):
    query = f"Give brief suggestions for managing and treating {request.disease}."
    answer = ask(query, retriever)
    return answer
...
async def chat(request: QueryRequest):
    full_query = (
        f"Disease context: {request.disease}\n"
        f"Previous conversation: {request.previousMessages}\n"
        f"User question: {request.message}"
    )
    answer = ask(full_query, retriever)
